<!-- /components/settings/Source/SourceForm.vue -->
<template>
  <div class="p-card p-p-4">
    ...
    <form class="flex flex-col gap-6" @submit.prevent="submitForm">
      <div class="flex gap-6" v-bind:class="(themeStore.isOnMobile)?'justify-center':'justify-end'">
        <div class="p-field">
          <label for="sourceName">Название</label>
          <InputText id="sourceName" v-model="newSource.name" required/>
        </div>
        <div class="p-field items-center">
          <label for="colorPicker">Цвет</label>
          <ColorPicker
              :pt:preview:style="`border: 1px solid var(--p-focus-ring-color)`"
              id="colorPicker" v-model="newSource.color"/>
        </div>
        <Button v-if="!themeStore.isOnMobile" icon="pi pi-plus" label="Добавить источник" type="submit" :loading="loading"/>
      </div>
      <Button v-if="themeStore.isOnMobile" icon="pi pi-plus" label="Добавить источник" type="submit" :loading="loading"/>
...
const submitForm = async () => {
  try {
    await settingsStore.createEntity("sources", newSource.value)
