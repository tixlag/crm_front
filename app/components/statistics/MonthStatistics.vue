...
const hasStatistics = computed(() => {
  return statisticsStore.month && statisticsStore.month.usersRequests && statisticsStore.month.usersRequests[0]
})

const tableUsersRequests = computed(() => {
  if (!statisticsStore.month) return []
  const totalSum = statisticsStore.month.usersRequests ? statisticsStore.month.usersRequests.reduce((a, b) => a + (b._sum.count || 0), 0) : 0;
  return statisticsStore.month.usersRequests.map(val => {
    return {
...
const tableTopByUrl = computed(() => {
  if (!statisticsStore.month) return []
  const totalSum = statisticsStore.month.topByUrl ? statisticsStore.month.topByUrl.reduce((a, b) => a + (b._sum.count || 0), 0) : 0;
  return statisticsStore.month.topByUrl.map(val => {
    return {
      name: getNameById(val.userId, 'managers'),
      count: val._sum.count,
      url: val.url,
...
//   if (!statisticsStore.month) return {}
//   return {
//     labels: statisticsStore.month.totalAmountByStatus.map(item => getNameById(item.statusId, 'orderStatuses')),
//     datasets: [
//       {
//         data: statisticsStore.month.totalAmountByStatus.map(item => item._sum.totalAmount),
//       }
//     ]
//   };
// });
...
//   if (!statisticsStore.month) return {}
//   return {
//     labels: statisticsStore.month.deliveryPriceByType.map(item => getNameById(item.deliveryTypeId, 'deliveryTypes')),
//     datasets: [
//       {
//         data: statisticsStore.month.deliveryPriceByType.map(item => item._sum.deliveryPrice),
//       }
//     ]
//   };
// });
