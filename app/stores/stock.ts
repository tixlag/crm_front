...
export const warehouseStockTypesShort = [
    { name: "Добавил", type: "ADD" },
    { name: "Переместил", type: "TRANSFER" },
    { name: "Удалил", type: "REMOVE" },
]
...
export const useStockLogsStore = defineStore('stockLogs', () => {
    ...
    const fetchStockLogs = async (page = 1, newLimit?: number, onlyMove = true ) => {
        loading.value = true
        error.value = null
        ...
        async function normalFilters(onlyMove = false) {
            ...
            if (!filtersToApply.activityType) filtersToApply.activityType = ['ADD', 'TRANSFER', 'REMOVE']
